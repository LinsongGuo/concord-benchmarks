...
perf_test() {
  ...
  do
    ...
    *)
    ...
    ACCURACY_TEST=$ACCURACY_TEST CONCORD_PASS_TYPE=$CONCORD_PASS_TYPE UNROLL_COUNT=$UNROLL_COUNT BUILD_LOG=$BUILD_DEBUG_FILE ERROR_LOG=$BUILD_ERROR_FILE PROFILE_FLAGS="-DAVG_STATS" make -f Makefile.ci ${bench}
    for thread in $THREADS
    do
      PER_THREAD_STAT_FILE="$DIR/parsec-perf_stats-th$thread-ad$AD.txt"
